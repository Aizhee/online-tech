$(document).ready(function(){
    ...
    $('.accordion-header').eq(0).addClass('accordion-open').find('i').removeClass('fa-plus').addClass('fa-minus'); // Change icon to fa-minus
    $('.accordion-body').eq(0).addClass('accordion-open-body');
    $('.accordion-header').click(function(){
        ...
        $('.accordion-body').not(accordionBody).removeClass('accordion-open-body');
        $('.accordion-header').not($(this)).removeClass('accordion-open').find('i').removeClass('fa-minus').addClass('fa-plus');

        // Toggle the accordion body of the clicked header
        accordionBody.slideToggle();
        accordionBody.toggleClass('accordion-open-body');

        // Toggle the icon class based on accordion body visibility
        $(this).toggleClass('accordion-open').find('i').toggleClass('fa-plus fa-minus');
    });
    ...
    var headerTopOuter = $('.header_top_outer');
    $(window).scroll(function(){
        if($(window).scrollTop() > 0){
            // Scrolling down
            headerTopOuter.addClass('add_shadow');
        } else {
    ...
    $(".slider_inner").slick({
      asNavFor: ".slider_nav",
      nextArrow:
        '<button class="slick-arrow slick-next"><img src="img/arrow-right.png" alt="next arrow"></button>',
      prevArrow:
        '<button class="slick-arrow slick-prev"><img src="img/arrow-left.png" alt="prev arrow"></button>'
    });
    ...
    $(".header_menu>ul").on("click", function () {
        if ($(window).width() <= 760) {
        $(".header_menu-btn").toggleClass("active"); 
        $(".header_menu>ul").slideToggle();
        }
    });

    //Fix the menu on resize
    $(window).on("resize", function () {
        if (($(window).width() >= 760)&&($(".header_menu>ul").is(":hidden"))) {
            $(".header_menu>ul").show();
            $(".header_menu-btn").removeClass("active");
             
        } else if (($(window).width() < 760)&&($(".header_menu>ul").is(":visible"))) {
            $(".header_menu-btn").removeClass("active");
            $(".header_menu>ul").hide();
        }
    });
    ...
    $('.works_item').hover(function() {
        ...
        // Destroy Slick Carousel when mouse leaves
        $(this).find('.carousely').slick('unslick');
      });

    });
        //Animations for icons
      $('.latest-link').hover(function() {
        $(':root').css('--gearAnim', 'rotateGear 2s linear infinite');
        }   , function() {
        // Pause the animation when mouse leaves
        $(':root').css('--gearAnim', 'none');
      });

      $('.top-link').hover(function() {
        $(':root').css('--tickAnim', 'enlargeTick .5s linear forwards');
        }   , function() {
        $(':root').css('--tickAnim', 'none');
      });

      $('.man-link').hover(function() {
        $(':root').css('--manAnim', 'slideUp .5s linear forwards');
        }   , function() {
